...
namespace Relisten.Import
{
    public class PhishNetImporter : ImporterBase
    {
        ...
        private static Regex PhishNetRatingScraper = new Regex(@">Rating:\s+<strong>\s+(?<AverageRating>[0-9.]+)<\/strong>\/5\s+\((?<VotesCast>[0-9]+)\s+votes\s+cast\)<");
        private static Regex PhishNetReviewCountScraper = new Regex(@"<span class=""attachedto"">attached to");

...
- all sync-visible identity and user-domain rows have explicit UUIDv7 identities plus natural unique constraints; the Timescale fact uses physical `(qualified_at, event_uuid)` while its ordinary receipt owns the global UUIDv7 primary key, and catalog references cross boundaries only as existing UUIDs;
...
Document the remaining boundary honestly: an admission policy can prevent unrelated `default` workloads from referencing account Secrets, but the trusted User Service deployer for its own allowlisted workload, cluster administrators, and nodes remain Secret-capable. Keep backup credentials out of `default`. Keep direct-primary EF migration Jobs operator-gated; do not grant current CI permission to create arbitrary Jobs in `default`. Rollout has two distinct promotions: an operator first runs the reviewed, uniquely named migration Job with the candidate immutable image digest while the Deployment stays on the old digest; only recorded Job success permits a second GitOps change that promotes that byte-for-byte digest to the Deployment. There is no runtime-image apply in the migration command and no startup migration fallback.
...
In the catalog API, add `POST https://api.relisten.net/api/v3/catalog/resolve` for standalone typed references such as favorites. An entity reference is exactly `{catalog_type, catalog_uuid}` and the launch allowlist is `artist`, `show`, `source`, `source_track`, `song`, `tour`, and `venue`. Accept at most 500 de-duplicated references. Return normalized UUID-bearing arrays for only the entities needed by the calling feature. Freeze collections as arrays in OpenAPI—no map representation or array-or-map union. Relationships contain UUIDs. Return typed availability such as `playable`, `removed`, or `upstream_missing`. This resolver is not part of playlist reads.
...
The catalog owns a monotonic global `availability_revision` that advances only when a catalog UUID's remotely playable/unavailable state changes. Favorite resolver responses and hydrated playlist snapshots include the revision used. A playlist snapshot includes both its content revision and `availability_revision`; its opaque `projection_revision` encodes the pair for sync freshness. It is not a byte-level HTTP validator for every catalog-sidecar title or venue. Playlist deltas advance only the playlist revision. If only availability changes, the next playlist read returns a new projection without synthesizing a user operation or fan-out writes. Add a full-representation ETag only if caching measurements later justify a referenced-entity digest or catalog projection version.
...
A cluster operator, not current application CI, applies the reviewed migration Job manifest for the candidate immutable digest while the runtime Deployment remains unchanged:
...
| Favorites | Desired-state exact retry has one effect; changed reuse conflicts; stale account generation cannot write | Add/remove offline, restart, reconnect, switch accounts, and confirm downloads remain global |
...
- EF migration Jobs use `identity.__EFMigrationsHistory`, direct-primary connections, unique release names, one operator at a time, and the same immutable image later deployed. Application pods never migrate.
...
Playlist/favorite snapshots return `availability_revision` and an opaque projection revision combining it with the domain revision. Library snapshot/delta responses also return all current followed-playlist heads even when the library cursor did not advance. Clients compare tokens for equality, not numeric ordering, and use the content revision for operation deltas versus the projection token for rendered-snapshot freshness.

Library `next_cursor` is opaque and scope/version bound. A cursor outside retained history returns HTTP `410 sync_cursor_expired` and requires `/v1/library/snapshot`; a playlist delta outside retained revisions returns HTTP `409 snapshot_required` and requires that playlist's snapshot. The favorite catalog resolver accepts at most 500 de-duplicated typed `entity_refs` using only `artist`, `show`, `source`, `source_track`, `song`, `tour`, and `venue`. Playlist snapshots hydrate through the User Service catalog projection and return normalized UUID-bearing arrays in the same response. The legacy Sonos numeric decoder is an inbound adapter compatibility shim and does not weaken this UUID-only contract.

...
The architecture contract for this work is [Relisten identity, user data, and Sonos architecture](../../architecture/2026-07-18-relisten-identity-user-data-and-sonos-architecture.md). [Deliver Relisten accounts from API to TestFlight](./2026-07-18-relisten-mobile-first-account-delivery-plan.md) is the execution source of truth for ordering work across `RelistenApi` and `relisten-mobile`, deciding what blocks a TestFlight build, and defining the testing and rollout policy. The mobile-specific implementation contract is [Relisten mobile accounts, library sync, and Sonos implementation](../../../../relisten-mobile/docs/plans/active/2026-07-18-relisten-mobile-accounts-library-sync-and-sonos-implementation.md). [Implement durable browser sessions for Relisten web](./2026-08-26-web-session-foundation.md) is the execution source of truth for the first credentialed Timber web slice. This file supplies the detailed server work within the mobile vertical slices. If the documents disagree about delivery order or initial scope, the mobile-first delivery plan wins for mobile work and the browser-session plan wins for its three-repository web scope. If implementation changes a durable architecture decision, update the relevant architecture document and this plan in the same pull request and record the reason below.
...
The work adds one ASP.NET Core `RelistenUserService` project to `RelistenApi`. The process serves `auth.relisten.net` and `accounts.relisten.net`. The `relisten.net` ingress also sends `/auth/session/*`, exact `/api/user/v1/csrf`, and the reviewed shared `/v1/me` and `/v1/library/*` routes to that process. The User Service owns PostgreSQL `identity` and `user_data`. The existing catalog API remains anonymous and owns catalog resolution. Temporal joins before the external-beta deletion gate; synchronous account and library writes remain ordinary PostgreSQL transactions and continue when Temporal is unavailable. The existing two-instance CloudNativePG cluster holds `app`, `temporal`, and `temporal_visibility`; this work does not create another CloudNativePG cluster or PostgreSQL pods.
...
- credentialed web login ends with an opaque, host-only, HttpOnly `relisten.net` session cookie and no browser-visible bearer token;
...
- [x] (2026-08-27) The first credentialed Timber foundation is implemented and locally verified. Its production rollout and Google proof remain approval-gated in [the browser-session ExecPlan](./2026-08-26-web-session-foundation.md).
...
- `/Users/alecgorge/code/relisten/relisten-web` is the Timber and Vite web app. It renders public playlist URLs and reaches `/auth/session/*`, exact `/api/user/v1/csrf`, and reviewed shared `/v1/me` and `/v1/library/*` routes through same-origin requests. It never stores a Relisten bearer token.
...
The target runtime topology is below. Timber provides same-origin reachability; the User Service remains the authorization boundary.
...
flowchart LR
    ...
    Web -->|"same-origin reviewed /v1"| Accounts
...
- `accounts.relisten.net` for native-bearer `/v1/*` resources plus one public-playlist read and the explicitly anonymous, no-store collaborator-invitation exchange;
- `relisten.net` for `/auth/session/*`, exact `/api/user/v1/csrf`, and reviewed shared `/v1/me` and `/v1/library/*` routes, plus the anonymous web paths added by later mobile slices.

In Development only, the configured loopback listener exposes the auth and Accounts routes needed by simulator and browser clients. Timber's fixed HTTPS origin proxies the reviewed browser families to that listener. The proxy grants reachability, not authority: API policies still require a native scope or persisted web capability, and unrelated `/v1/*` routes remain unavailable to web sessions. Keep loopback and development hostnames on an explicit allowlist; do not disable Host Filtering.
...
Each web session persists a fixed capability set rather than translating every native scope. Reviewed shared routes accept exactly one credential: native bearer requests require native scopes, and web-cookie requests require persisted capabilities. A request that presents both credentials fails. One global unsafe-cookie-request boundary requires the exact Origin and session-bound antiforgery token. New or unreviewed `/v1/*` routes remain bearer-only until their browser policy and capability are reviewed. Web capabilities exclude `sonos.control`, internal adapter scopes, group discovery, handoff, and playback commands.
...
Keep the User Service as the web session authority without making authenticated Timber product UI a dependency of the mobile slices. Browser lifecycle routes, exact `/api/user/v1/csrf`, and reviewed shared `/v1/me` and `/v1/library/*` resources exist as the foundation. Build only the public page at `/p/{public_code}` with its `/api/public-playlists/*` path in Slice 5, the invitation fallback when Slice 6 requires it, and callback or disconnect pages that an external provider or Sonos flow strictly requires. Everything else remains Timber-owned or returns 404. The public page needs no cookie or grant. Do not add a browser token manager.

The shared resource contract uses fixed web-session capabilities and explicit browser-capable policies. Native scopes and web capabilities remain distinct, and browser-cookie mutations inherit global Origin and antiforgery enforcement. The mobile-first phase still implements only the public, callback, and disconnect routes that its slices need; it does not require Timber product screens. Focused route-isolation tests prove that native Sonos groups, handoffs, playback handles, commands, internal routes, and other unreviewed `/v1/*` endpoints remain unavailable to web sessions.
...
Skip this section for a mobile slice that does not change `relisten-web`. In Slice 5, validate only the public-playlist fallback and callback/link behavior owned by that slice. The linked browser-session ExecPlan owns the durable credentialed-web checks.
...
For Slice 5, manually verify exact callbacks, public snapshot cache/revalidation, uniform public 404s, automatic playlist hydration, and anonymous catalog operation with the User Service stopped. The browser-session checks cover native compatibility on shared routes, web capabilities, dual-credential rejection, global cookie Origin and antiforgery enforcement, route isolation, and absence of browser token storage. Test physical-device universal/app-link routing before the relevant release.
...
| `relisten.net` | `/auth/session/*`, exact `/api/user/v1/csrf`, reviewed `/v1/me` and `/v1/library/*`, mobile callbacks, public playlist page, and invitation exchange | exact anonymous route policy or opaque web session with route-specific capabilities |
...
iOS and Android use authorization code with PKCE as public clients. The adapter alone uses client credentials. Canonical `sub` is the lowercase hyphenated Relisten user UUID. Native access tokens target the accounts audience and include `sub`, client, `sid`, `security_version`, expiry, and scopes. Refresh and resource requests load the current session/user so revocation, disable, and deletion take effect. The confidential web client also uses authorization code with PKCE, exchanges server-side, accepts identity only from the maintained pipeline's fully validated Relisten ID token, and creates an opaque session without retaining bootstrap tokens.

The web callback's bootstrap credential has no accounts-resource audience and is discarded after creating the opaque session. Its session receives only reviewed web capabilities and never `sonos.control`. Browser switch-account revokes the web and linked auth-SSO sessions before restarting provider selection. Adapter credentials have audience `https://accounts.relisten.net/internal/sonos`, only the named `sonos.smapi`, `sonos.cloudqueue.read`, `sonos.playback.report`, and `sonos.events.write` scopes needed by that adapter client, and no user `sub`/`sid`. Provider subject/email remain server-side. Native account switching separately stops playback, clears queues, makes one bounded revocation attempt, deletes the old local credential, and then changes scope.

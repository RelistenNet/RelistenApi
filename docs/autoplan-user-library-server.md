...
- [ ] Continue sync/favorites/settings with playlist, invitation, and revocation sync aggregation before playback-history promotion.
- [ ] Promote playback-history after sync/favorites/settings covers the full Milestone 4 sync contract.
...
| sync-favorites-settings | active | root Codex agent | none | `docs/workstreams/active/sync-favorites-settings/plan.md` | `docs/workstreams/active/sync-favorites-settings/ledger.md` | branch `codex/user-library-sync` | SYNC-001 complete; implement SYNC-002 for playlist, invitation, and revocation sync aggregation. | `continue` |
...
The foundation, auth/session, playlist aggregate, share-token/mobile-access, source-range, reorder, clone, collaborator-invitation, public cache/read-contract, and favorites/settings sync-base slices are complete. The sync contract should continue with playlist, invitation, and revocation entries before playback-history work starts. Sync cursors should remain sequence-backed rather than timestamp-watermark based. Keep local mobile development pointed at separate base URLs: catalog API at `http://localhost:3823/api` and user-library API at `http://localhost:5119`. Provider credential files are local/dev inputs only and must stay out of repo source.

## Next Iteration

Run `SYNC-002` in `sync-favorites-settings`: aggregate playlist, collaborator invitation, follower, and revocation changes into the sync feed with tombstones where needed, then prove mobile-relevant incremental behavior with high-signal endpoint tests.

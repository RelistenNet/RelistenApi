...
The foundation, auth/session, live Apple/Google OIDC provider verification, recent-reauth account gates, playlist aggregate, share-token/mobile-access, source-range, reorder, clone, collaborator-invitation, public cache/read-contract, sync/favorites/settings, playback-history batch-ingest, recent-history read, anonymous catalog aggregate queue, contract baseline, separate user API image path, and account export/deletion gates are complete for M1 server behavior. Sync cursors should remain sequence-backed rather than timestamp-watermark based. Keep local mobile development pointed at separate base URLs: catalog API at `http://localhost:3823/api` and user-library API at `http://localhost:5119`. Provider credential files are local/dev inputs only and must stay out of repo source.

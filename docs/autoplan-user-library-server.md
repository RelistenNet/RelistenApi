...
- [x] 2026-06-20T04:27:04Z Started playback-history second iteration on branch `codex/user-library-history-read`.
- [x] 2026-06-20T04:29:14Z Completed playback-history second iteration with a capped recent-history read endpoint, current-user scoping, minimal response DTO, high-signal tests, and validation evidence.
- [x] 2026-06-20T04:37:47Z Completed playback-history third iteration with an anonymous catalog aggregate queue, accepted-only enqueue behavior, retry/disabled no-op coverage, schema smoke, and validation evidence.
- [x] 2026-06-20T04:43:12Z Promoted server-contract hardening on branch `codex/user-library-contract-hardening` and added no-store/schema-placement contract tests with validation evidence.
...
| server-contract-tests | active | root Codex agent | account deletion/export gates remain | `docs/workstreams/active/server-contract-tests/plan.md` | `docs/workstreams/active/server-contract-tests/ledger.md` | branch `codex/user-library-deployment-hardening` | CT-002 deployment artifact complete; next add account deletion/export server support. | `done` |

## Current Hypothesis

The foundation, auth/session, playlist aggregate, share-token/mobile-access, source-range, reorder, clone, collaborator-invitation, public cache/read-contract, sync/favorites/settings, playback-history batch-ingest, recent-history read, anonymous catalog aggregate queue, contract baseline, and separate user API image path are complete for M1. Remaining release-gate work is account deletion/export server support and any final runtime documentation. Sync cursors should remain sequence-backed rather than timestamp-watermark based. Keep local mobile development pointed at separate base URLs: catalog API at `http://localhost:3823/api` and user-library API at `http://localhost:5119`. Provider credential files are local/dev inputs only and must stay out of repo source.

## Next Iteration

Commit CT-002, then implement account deletion/export server gates.
...
2026-06-20: Milestone 5 recent-history read slice completed on branch `codex/user-library-history-read`. It adds authenticated `GET /api/v3/library/history/recent` with capped limit validation, current-user scoping, newest-first ordering, and a minimal response DTO for playback identity and playlist attribution. Validation passed for focused history tests 6/6, `RelistenUserApiTests` 69 tests, existing `RelistenApiTests` 47 tests, full solution build with 0 warnings, `git diff --check`, and secret-path scan. Root review removed ingest-only device/app metadata from the response contract; reviewer feedback added a raw JSON assertion for omitted metadata and snake-case attribution fields.

2026-06-20: Milestone 5 catalog aggregate queue slice completed on branch `codex/user-library-history-aggregate`. It adds `user_data.playback_history_catalog_play_queue`, a small `PlaybackHistoryCatalogAggregateSink`, transactional enqueue after accepted history inserts, retry-safe dedupe through `playback_history_id`, and disabled-history no-op behavior. Validation passed for focused history tests 6/6, `RelistenUserApiTests` 69 tests, existing `RelistenApiTests` 47 tests, full solution build with 0 warnings, and local Postgres schema smoke for migration marker 9, the queue table, and the unprocessed index. Root review kept the user API from writing directly to `public.source_track_plays` and kept personal/user/device fields out of the aggregate queue. Queue drain/retry semantics and backfill for already-accepted historical rows remain out of scope for this slice.

2026-06-20: Milestone 6 server-contract hardening baseline completed on branch `codex/user-library-contract-hardening`. It adds `UserLibraryContractTests` for no-store defaults across representative authenticated read endpoints and real Postgres schema placement for user-owned tables. Validation passed for focused contract tests 8/8, `RelistenUserApiTests` 77 tests, existing `RelistenApiTests` 47 tests, full solution build with 0 warnings, `git diff --check`, and secret-path scan. Reviewer found no code/test correctness issues and stale docs were corrected.

2026-06-20: Milestone 6 deployment hardening slice completed on branch `codex/user-library-deployment-hardening`. It adds `Dockerfile.userapi`, a separate manual GitHub Actions image workflow, `deploy-user-api`, README local smoke/deploy commands, and Docker context cleanup. Validation passed for release publish, focused contract tests 8/8, full solution build with 0 warnings, Docker image build, container `/health` smoke with schema initialization disabled, workflow YAML parse, `git diff --check`, and secret-path scan. Reviewer findings were fixed with deploy concurrency, pinned action refs for secret-sensitive setup, base64-decoded kubeconfig setup preserving the existing secret contract, direct kubectl rollout commands, and rollout status waiting; final reviewer verification found no workflow blockers.

...
The production permissions SQL revokes `CREATE` on `public` from the database-wide `PUBLIC` role and sets default privileges explicitly so new user tables are not accidentally exposed. The migration Job applies this SQL after EF creates or upgrades the owned objects. Keep the permissions SQL versioned with the application release; do not duplicate it in CloudNativePG manifests or local setup.
...
The catalog owns a monotonic, global `availability_revision` that advances only when the remotely playable/unavailable state of a UUID changes. Playlist hydration and the standalone favorite resolver return it, and each playlist/favorite snapshot reports the revision used to construct its active projection. The playlist projection token combines playlist revision with `availability_revision`; clients compare it for freshness rather than coordinating resolver chunks. This deliberately trades a rare coarse invalidation for no cross-schema fan-out and a testable removal contract.
...
The User Service uses the primary through the read-write transaction-mode PgBouncer endpoint for both reads and writes. Authentication, revocation, playlist revisions, operation serialization, and immediate post-login reads must not depend on replica lag. Start each replica with an Npgsql maximum pool size of 10, account for dispatcher and worker pools separately, and measure PgBouncer queue time before raising any pool.
...
Application replicas never run migrations at startup. Build and sign one immutable `sha-*` release image. An operator-gated workflow—not the ordinary `default` namespace application deployer—creates a uniquely named migration Job using `user_service_migrator` and that exact image. The Job connects directly to the CloudNativePG primary Service, not transaction-mode PgBouncer, runs EF Core, then applies the release's production permissions SQL. Only after it succeeds may CI apply the Deployment with the same digest; a failed Job stops rollout. Migrations remain backward compatible with the currently running release until the new pods are ready.

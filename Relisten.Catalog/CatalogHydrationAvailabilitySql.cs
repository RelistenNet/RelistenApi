namespace Relisten.Catalog;

/// <summary>
/// Builds the catalog checks shared by favorite acceptance and mobile metadata hydration.
/// A reference is available only when the catalog can return its ordinary DTO and required parents.
/// </summary>
public static class CatalogHydrationAvailabilitySql
{
    public static string BuildResolution(IReadOnlySet<string> requestedTypes) => $@"
{BuildAvailableReferencesCte(requestedTypes, includeOrdinal: true)}
SELECT
    r.catalog_type,
    r.catalog_uuid,
...
ORDER BY r.ordinal;";
    ...
    public static string BuildAvailableReferences(IReadOnlySet<string> requestedTypes) => $@"
{BuildAvailableReferencesCte(requestedTypes, includeOrdinal: false)}
SELECT catalog_type, catalog_uuid
FROM available;";

    private static string BuildAvailableReferencesCte(
        IReadOnlySet<string> requestedTypes,
        bool includeOrdinal)
    {
        var checks = AvailabilityChecks
            .Where(check => requestedTypes.Contains(check.Key))
            .Select(check => check.Value);
        var requestedColumns = includeOrdinal
            ? "catalog_type, catalog_uuid, ordinal"
            : "catalog_type, catalog_uuid";
        var ordinality = includeOrdinal ? " WITH ORDINALITY" : string.Empty;
    ...
    ){ordinality} AS requested({requestedColumns})
), available AS (
    {string.Join("\n    UNION ALL\n    ", checks)}
)";
    }

    private static readonly IReadOnlyDictionary<string, string> AvailabilityChecks =
        new Dictionary<string, string>
        {
            ["artist"] = @"SELECT r.catalog_type, r.catalog_uuid
    FROM requested r
    ...
    JOIN features feature ON feature.artist_id = entity.id",
            ["show"] = @"SELECT r.catalog_type, r.catalog_uuid
    FROM requested r
    ...
    JOIN years effective_year
        ...
        AND (
            ...
            ["source"] = @"SELECT r.catalog_type, r.catalog_uuid
    FROM requested r
    ...
    JOIN features source_feature ON source_feature.artist_id = source_artist.id",
            ["source_track"] = @"SELECT r.catalog_type, r.catalog_uuid
    FROM requested r
    ...
    WHERE entity.is_orphaned = false
        AND (entity.mp3_url IS NOT NULL OR entity.flac_url IS NOT NULL)",
            ["song"] = @"SELECT r.catalog_type, r.catalog_uuid
    FROM requested r
    JOIN setlist_songs entity ON r.catalog_type = 'song' AND entity.uuid = r.catalog_uuid
    JOIN artists artist ON artist.id = entity.artist_id
    JOIN features feature ON feature.artist_id = artist.id",
            ["tour"] = @"SELECT r.catalog_type, r.catalog_uuid
    FROM requested r
    ...
    WHERE entity.start_date IS NOT NULL AND entity.end_date IS NOT NULL",
            ["venue"] = @"SELECT r.catalog_type, r.catalog_uuid
    FROM requested r
